...
function MinistryCard({ m, index }) {
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row">
      {/* Image Side */}
      <div className={`w-full md:w-2/5 relative min-h-[250px] ${isEven ? 'order-1' : 'order-1 md:order-2'}`}>
        <img
          src={m.imagen}
          alt={m.titulo}
      ...
      {/* Content Side */}
      <div className={`w-full md:w-3/5 p-8 md:p-12 flex flex-col justify-center ${isEven ? 'order-2' : 'order-2 md:order-1'}`}>
